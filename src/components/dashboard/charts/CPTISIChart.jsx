...
export function CPTISIChart({ data }) {
  const chartData = [
    { label: 'Short ISI under 1.75s', rt: data.overall.shortISImeanRTms },
    { label: 'Long ISI, 1.75s or longer', rt: data.overall.longISImeanRTms },
  ]
  return (
    <figure aria-label="Bar chart comparing mean RT for short and long ISI trials">
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={chartData} layout="vertical">
          ...
          <YAxis type="category" dataKey="label" width={176} tick={{ fill: CHART_DEFAULTS.tickFill, fontSize: CHART_DEFAULTS.tickFontSize }} />
          <Tooltip
