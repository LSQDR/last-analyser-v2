import { getInterferenceBand } from '../../utils/compute/computeStroopMetrics.js'

const BANDS = ['minimal', 'typical', 'elevated', 'high']
const BAND_LABELS = { minimal: 'Minimal', typical: 'Typical', elevated: 'Somewhat Elevated', high: 'High' }
  ...
  return (
    <div className="interference-band" role="img" aria-label={`Interference band: ${band.label}`}>
      <div className="band-track">
        {BANDS.map(b => (
          <div key={b} className={`band-segment band-segment--${b} ${b === band.band ? 'band-segment--active' : ''}`}
            aria-hidden="true">
