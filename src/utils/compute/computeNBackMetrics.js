...
function evaluateFlags(correctedHitRate) {
  if (correctedHitRate < 40) return ['severeWorkingMemoryDifficulty']
  if (correctedHitRate < 60) return ['lowCorrectedHitRate']
  return []
}

export function computeNBackMetrics(events) {
  const scored = events.filter(e => e.scored)
  ...
  const hitRate         = getPercentage(hits.length,        targets.length)
  const falseAlarmRate  = getPercentage(falseAlarms.length,  nonTargets.length)
  const correctedHitRate = hitRate - falseAlarmRate
