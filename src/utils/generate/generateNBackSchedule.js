...
const cfg = TASK_REGISTRY.find((t) => t.id === 'matchOrPass').config

export const DEFAULT_NBACK_CONFIG = {
  colours: cfg.colours,
  warmupTrials: cfg.warmupTrials,
  twoBackWarmupTrials: 6,
  scoredTrials: cfg.scoredTrials,
  scoredN: cfg.scoredN,
  targetRatio: cfg.targetRatio,
  stimulusMs: cfg.stimulusMs,
  isiMs: cfg.isiMs,
  maxConsecutiveTargets: cfg.maxConsecutiveTargets,
  maxConsecutiveNonTargets: cfg.maxConsecutiveNonTargets,
}
...
function generateNBackSequence(n, totalTrials, targetRatio, config) {
  ...
  for (let i = n; i < totalTrials + n; i++) {
    ...
    if (isTarget) {
      ...
      do {
        colour = randomFrom(config.colours)
      } while (colour === f || colour === prev)
